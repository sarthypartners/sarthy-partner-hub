...
| Communication Template Library | Partnership SOP #16 (the link, not the content), #17 (Reactivation), Combined Journey Map (Findings 1 and 2 cite specific message triggers directly - re-check both findings if any of the 11 triggers change) |
...
| Partnership SOP §7 (Path A / Path B diagram) | Business Requirements Document §5 and Data Dictionary - both point to this diagram instead of redrawing it. Combined Journey Map - its Stage 1 (Origination) is built directly from this diagram |
| Combined Journey Map | Nothing yet - this is step one of a planned partner-engagement improvement effort, diagnostic only. Once actual fixes are designed (new message templates, a startup-side tracking mechanism), check them back against this map's three findings before treating either as settled |
