...
| Partnership SOP §7 (the 3-direction connection flow, formerly the Path A / Path B diagram) | Business Requirements Document §5 and Data Dictionary - both point to this section instead of redrawing it. Combined Journey Map - its Stage 1 (Origination) was built from the old Path A/B diagram and needs a pass to match the confirmed 3-direction model. Connection Request Flow - this is now the same rule described two ways (the detailed walkthrough vs. the authoritative SOP text); keep both in sync |
| Combined Journey Map | Nothing yet - this is step one of a planned partner-engagement improvement effort, diagnostic only. Once actual fixes are designed (new message templates, a startup-side tracking mechanism), check them back against this map's three findings before treating either as settled. The Engagement Strategy Framework is built directly from its 3 findings - if a finding changes, check the framework's matrix still matches it. Also now stale against the confirmed 3-direction connection flow (Stage 1 was written against the old Path A/B model) - needs a pass |
